...
debug( "fileCount : " 7zipHandler.getFileCount( fileName ) )
debug( "version   : " 7zipHandler.getVersion() )

msgbox % "list      : " 7zipHandler.list( fileName )
; debug( "list      : " 7zipHandler.list( fileName ) )

debug( A_ScriptDir "\temp" )



7zipHandler.extract( fileName, A_ScriptDir "\temp" )
...
class 7Zip {
  ...
  ; Related: 7Zip_Update , 7Zip_Delete
  ;
  extract( srcPath, trgPath="" ) {
    commandline := this.op.ExtractPaths ? "x """ . srcPath . """" : "e """ . srcPath . """"
    commandline .= this.op.Hide ? " -hide" : ""
    commandline .= this._recursion()
    if ( trgPath != "" ) {
      FileCreateDir, % trgPath
      commandline .= " -o""" . trgPath . """"
    } else {
      if ( this.op.Output != "" ) {
        FileCreateDir, % this.op.Output
        commandline .= " -o""" . this.op.Output . """"
      }
    }
  ...
  setOwnerWindowEx( sProcFunc ) {
    ...
    Return DllCall( 7Zip.gop.dllName "\SevenZipSetOwnerWindowEx","Ptr", 0 , "ptr", Address )
  }
  ...
  ;
  killOwnerWindowEx() {
    Return DllCall( 7Zip.gop.dllName "\SevenZipKillOwnerWindowEx" , "Ptr", 0 )
  }
  ...
  ;
  configDialog() {
    Return DllCall( 7Zip.gop.dllName "\SevenZipConfigDialog", "Ptr", 0, "ptr",0, "int",0 )
  }
  ...
  openArchive( sArcName ) {
    Return DllCall( 7Zip.gop.dllName "\SevenZipOpenArchive", "Ptr", 0, "AStr", sArcName, "int", 0 )
  }
  ...
  ;
  closeArchive( sArcName ) {
    Return DllCall( 7Zip.gop.dllName "\SevenZipCloseArchive", "Ptr", 0, "AStr", sArcName, "int", 0 )
  }
  ...
  _run( sCommand ) {
    ...
    aRet := DllCall(7Zip.gop.dllName "\SevenZip", "Ptr", ""
            ,"AStr", sCommand
            ,"Ptr", &tOutBuffer
            ,"Int", nSize)
    If !ErrorLevel
