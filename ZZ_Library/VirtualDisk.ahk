...
class VirtualDisk {
  ...
  static _init() {
      ;VirtualDisk.daemonPath := "c:\Program Files (x86)\DAEMON Tools Lite\DTLite.exe"
      ; VirtualDisk.daemonPath := "c:\Program Files\DAEMON Tools Lite\DTAgent.exe"
      VirtualDisk.daemonPath   := "c:\Program Files\DAEMON Tools Lite\DTCommandLine.exe"
      VirtualDisk.DRIVE_LETTER := "G"
  }
  static void := VirtualDisk._init()

  __New() {
    throw Error("VirtualDisk is a static class, dont instante it!", -1)
  }

  open(filePath, showError := false) {
    if (!FileExist(this.daemonPath)) {
      if (showError == true) {
    ...
    if (!InStr(fileExtWanted, fileExt)) {
      if (showError == true) {
    ...
    if (!result) {
      errorMsg := "VirtualDisk does not work properly for mounting cd image file [" . filePath . "]."
      this.close()
      MsgBox(errorMsg)
      return false
    }
  ...
  close() {
    ...
    if (result == false) { 
      errorMsg := "VirtualDisk does not unmount"
      debug(errorMsg)
      MsgBox(errorMsg)
      return false
    }
